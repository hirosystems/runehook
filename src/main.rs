...
fn test_postgres() -> Result<postgres::Client, postgres::Error> {
    ...
    client.batch_execute("
        CREATE TABLE etching (
            divisibility    INT,
            premine         NUMERIC,
            rune            NUMERIC NOT NULL,
            rune_display    TEXT NOT NULL PRIMARY KEY,
            spacers         NUMERIC,
            symbol          TEXT,
            amount          NUMERIC,
            cap             NUMERIC,
...
fn insert_etching(etching: &Etching, client: &postgres::Client) -> Result<bool, postgres::Error> {
    ...
    let Some((terms_height_a, terms_height_b)) = etching.terms.map(|t| (t.height.0.unwrap(), t.height.1.unwrap())) else {
        println!("etching.terms = null");
        return Ok(false)
    };
    let Some((terms_offset_a, terms_offset_b)) = etching.terms.map(|t| (t.height.0.unwrap(), t.height.1.unwrap())) else {
        println!("etching.terms = null");
        return Ok(false)
    };    
    ...
    client.execute(
        ...
        &[&divisibility, &premine, etching.rune.map(|r| r.0), &run_display, &etching.spacers, etching.symbol, terms_amount, terms_cap, terms_height_a, terms_height_b, terms_offset_a, terms_offset_b, etching.turbo],
    )?;
